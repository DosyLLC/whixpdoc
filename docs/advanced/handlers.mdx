---
title: Building logic with Handlers
version: 1
---
...
whixp.addEventHandler<TransportState>('state', (state) {
    if (state == TransportState.connecting) {
      ...
      log('Connection failure occurred');
...
Handler attached to this event will be processed when an active XMPP session is terminated.
