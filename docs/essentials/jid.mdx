...
These preparations map characters and disallow character categories according to [RFC 3454](https://datatracker.ietf.org/doc/html/rfc3454).

## JID Components

A JID consists of three main components:

1. **Node**: The username or identifier before the "@" symbol (e.g., "vsevolod" in "vsevolod@dosy.app")
2. **Domain**: The server domain after the "@" symbol (e.g., "dosy.app")
3. **Resource**: An optional identifier after the "/" symbol that distinguishes multiple connections from the same user (e.g., "desktop" in "vsevolod@dosy.app/desktop")
